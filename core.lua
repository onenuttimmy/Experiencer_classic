...
LibSharedMedia:Register("font", "DorisPP", [[Interface\AddOns\Experiencer_classic\media\DORISPP.TTF]]);
...
function Addon:OpenContextMenu(anchorFrame, clickedModuleIndex)
    ...
    -- Fix: Use proper anchoring point on the helper frame
    local point, relativePoint, sign = Addon:GetAnchors(anchorFrame);
    
    Addon.ContextMenu:ClearAllPoints();
    Addon.ContextMenu:SetPoint(point, anchorFrame, relativePoint, 0, 0);

    -- Fix: Anchor the menu to the frame, NOT the cursor
    EasyMenu(menudata, Addon.ContextMenu, Addon.ContextMenu, 0, 5 * sign, "MENU");
end
